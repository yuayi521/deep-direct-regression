...
def classification_net(input_tensor=None, trainable=False):
    ...
    x = Convolution2D(1, (1, 1), strides=(1,1), padding='same')(upscore16)
    #x = Dense(1)(x)
    return x
...
def regression_net(input_tensor=None, trainable=False):
    ...
    #??
    bn_axis = 3
    x = Convolution2D(32, (5, 5), strides=(1,1), padding='same', activation='relu')(img_input)
    ...
    #x = Convolution2D(1, (1, 1), strides=(1,1), padding='same', activation='sigmoid')(x)
    x = Convolution2D(128, (1, 1), strides=(1,1), padding='same', activation='relu')(x)
    x = Convolution2D(8, (1, 1), strides=(1,1), padding='same', activation='relu')(x)
    #x = Dense(1)(x)
    return x
...
X_train, Y_train, Y_train_regr, Z  = data_gen_train.next()

for i in xrange(6000):
    X_train_iter, Y_train_iter, Y_train_regr, Z  = data_gen_train.next()
    X_train = np.concatenate([X_train, X_train_iter],axis = 0) 
    Y_train = np.concatenate([Y_train, Y_train_iter],axis = 0) 

loss_class = clas_model.fit(X_train, Y_train, batch_size=100, epochs=500,verbose=1)
