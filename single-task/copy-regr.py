...
def classification_net(input_tensor=None, trainable=False):
    ...
    x = Convolution2D(1, (1, 1), strides=(1,1), padding='same', activation='sigmoid')(upscore16)
    #x = Dense(1)(x)
    return x
...
def regression_net(input_tensor=None, trainable=False):
    ...
    #x_regr = Activation('linear')(x)
    #x_regr = Activation('softmax')(x)  
...
#2) define network
clas = classification_net(img_input,trainable=True)
regr = regression_net(img_input,trainable=True)

#clas_model = Model(img_input,clas)
#print clas_model.summary()
#clas_model.compile(loss='hinge', optimizer='sgd')
...
Y_train = np.concatenate([Y_train_regr, Y_train_cls], axis = 3)
for i in xrange(7):
    Y_train = np.concatenate([Y_train, Y_train_cls], axis = 3)

for x in xrange(1000):
    X_train_iter, Y_train_cls, Y_train_regr, Z  = data_gen_train.next()
    X_train = np.concatenate([X_train, X_train_iter], axis = 0)
    Y_train_iter = np.concatenate([Y_train_regr, Y_train_cls], axis = 3)
    for i in xrange(7):
        Y_train_iter = np.concatenate([Y_train_iter, Y_train_cls], axis = 3)
    Y_train = np.concatenate([Y_train, Y_train_iter], axis = 0)
    #loss_regr = regr_model.train_on_batch(X_train,Y_train_merge)
    #loss_cls = clas_model.train_on_batch(X_train,Y_train_cls)
    #print loss_cls

loss_class = regr_model.fit(X_train, Y_train, batch_size=10, epochs=500,verbose=1)
